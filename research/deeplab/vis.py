...
def _process_batch(sess, original_images, semantic_predictions, image_names,
    ...
    for i in range(num_image):
        ...
        crop_semantic_prediction = semantic_prediction[:image_height, :image_width] if not FLAGS.save_raw_logits \
            else semantic_prediction[:image_height, :image_width, :]

        # Save image.
        if not FLAGS.save_raw_logits:
...
def main(unused_argv):
    ...
    with g.as_default():
        ...
        if tuple(FLAGS.eval_scales) == (1.0,):
            ...
            predict = model.predict_logits if FLAGS.save_raw_logits else model.predict_labels
            predictions = predict(
        ...
        if FLAGS.min_resize_value and FLAGS.max_resize_value:
            ...
            # TODO: Prediction rank 4 if saving logits. Do I have to worry about how interpolation is done during resizing?
            if not FLAGS.save_raw_logits:
                ...
                embedding_dim = predictions.shape(3)
                predictions = tf.slice(
                    predictions,
                    [0, 0, 0, 0],
                    [1, original_image_shape[0], original_image_shape[1], embedding_dim])
                resized_shape = tf.to_int32([tf.squeeze(samples[common.HEIGHT]),
                                             tf.squeeze(samples[common.WIDTH])])
                predictions = tf.image.resize_images(predictions,
                                                     resized_shape,
                                                     method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
                                                     align_corners=True)
